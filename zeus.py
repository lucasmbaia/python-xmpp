...
class Zeus(sleekxmpp.ClientXMPP):
	...
	def _append_deploy_application(self, args, iq_response, ifrom):
		logging.info('Start Append Deploy Application %s' % (args['application_name']))
		...
		try:
			self._request_deploy_minion(args=args, image_name=values_application['image'], iq_response=iq_response, ifrom=ifrom, key_application=key_application, total_containers=args['total_containers'], start_count=start_count)
		except Exception as e:
			raise Exception(e)
	...
	def _request_deploy_minion(self, args, image_name, iq_response, ifrom, key_application, total_containers, first=False, start_count=1):
		if len(self.minions) == 1:
			...
			if first:
				try:
					self._generate_image(path=args['path'], image_name=image_name, key_application=key_application, ito=keys_minions[0])
				except Exception as e:
					raise Exception(e)

				for minion in keys_minions[1:]:
					try:
						path = '{0}{1}.tar.gz'.format(self.path_images, image_name)
...
#                iterator += minions_pods[key]
	...
	def append_containers(self, msg):
		...
		for op in options:
			...
			if "--total" in op:
				args['total_containers'] = int(op.replace("--total=", "").strip())
		...
		if 'application_name' not in args:
			self._handler_send_message(msg['from'], 'Name of application is required')
			return

		if 'total_containers' not in args:
			self._handler_send_message(msg['from'], 'Total of containers is required')
			return

		try:
			self._append_deploy_application(args=args, iq_response=None, ifrom=msg['from'])
		except IqError as e:
			self._handler_send_message(msg['from'], unicode(e.iq['error']['text']))
		except IqTimeout as t:
			self._handler_send_message(msg['from'], unicode(t))
#		options = msg['body'].split('%')
#
