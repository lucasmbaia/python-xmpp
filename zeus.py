...
class Zeus(sleekxmpp.ClientXMPP):
    ...
    def _handler_deploy(self, iq):
        ...
        if 'master-first-deploy' in iq['id']:
            ...
            try:
                response = self._deploy_application(args=args, iq_response=iq['id'], ifrom=iq['from'])
            except Exception as e:
	...
	try:
	    self._request_deploy_minion(args=args, image_name=image_name, iq_response=iq_response, ifrom=ifrom, key_application=key_application, first=True)
	except Exception as e:
	    raise Exception(e)
	...
	if len(self.minions) == 1:
	    ...
	    if first:
		try:
		    self._generate_image(path=args['path'], image_name=image_name, key_application=key_application, ito=self.keys_minions[0])
		except Exception as e:
		    raise Exception(e)
		...
		for count in range(total_containers):
		    container_name = '{0}-{1}'.format(application_name, str(iterator))
		    iterator += 1
		...
		thread_start_deploy = threading.Thread(target=start_deploy, args=[self, minion, minions_containers[minion], iterator, key_application, args['application_name'], ifrom])
...
#                iterator += minions_pods[key]
    ...
    def _requet_deploy_to_minion(self, ito, application_name, key_application, container_name, ifrom):
        try:
            ...
            self._handler_send_message(ifrom, 'timeout container ' + container_name)
