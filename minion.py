...
class Minion(sleekxmpp.ClientXMPP):
			...
			if 'die' in ev['docker']['status']:
				if container_name is not None:
					if ev['args']['container_process'] in self.container_deploy_start:
						self.container_deploy_start.remove(ev['args']['container_process'])
					self.channel_connections[ev['args']['container_process']].close()
	...
	def _load_image(self, path):
		...
		try:
			exists_image = self.docker_commands.exist_image(image_name=image_name)
		except Exception as e:
			raise Exception(e)
