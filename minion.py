...
class Minion(sleekxmpp.ClientXMPP):
	...
	def _handler_docker(self, iq):
		print(iq)
		if 'name-containers' in iq['id']:
			print("CARALHO")
			try:
				print("VAI TOMA NO CU")
				containers = self.docker_commands.name_containers()
				...
				self.plugin['docker'].response_containers_minion(iq_response=iq['id'],
																ito=iq['from'],
																ifrom=self.boundjid,
																success=True,
																response=','.join(str(s) for s in containers))
	...
	def _check_container_die(self, event):
		...
		if container_name in self.minion_containers:
			...
			print(customer, application_name, key_application)
			print(self.retry_deploy_container[container_name])
			if self.retry_deploy_container[container_name] < 1:
				self._thread_generate_container_die(application_name=application_name,
													container_name=container_name,
													key_application=key_application,
													retry=self.retry_deploy_container)

				self.retry_deploy_container[container_name] += 1
			else:
				print("VAIDA")
				die_msg = 'container-die {0} {1} {2} {3}'.format(str(application_name), str(container_name), self.boundjid, str(key_application))

				print(self.zeus, die_msg)

				self.send_message(mto=self.zeus,
								mbody=die_msg,
								mtype='chat')

				del self.minion_containers[container_name]

	def _thread_generate_container_die(self, application_name, container_name, key_application, retry):
		logging.info('Start thread to create container die: %s' %
					 (container_name))

		try:
			values = ast.literal_eval(self.etcd_conn.read(key_application))
		except Exception as e:
			logging.error('Error to read key % in etcd: %s' %
						  (key_application, unicode(e)))
			return

		self.haproxy.remove_container(application_name=str(application_name), container_name=str(container_name), protocol=ast.literal_eval(str(values['protocol'])))

		container_process = '{0}-{1}'.format(str(container_name), str(retry))

		args = {'application_name': str(application_name), 'container_name': str(container_name), 'protocol': str(values['protocol']), 'dns': str(values['dns']), 'container_process': str(container_process)}

		self.container_deploy_start.append(container_process)
		self.channel_connections[container_process] = Channel(server_process=self.docker_process,
															  pod_id=container_name,
															  pod_args=args)

		self.channel_connections[container_process].register(self.docker_process.public_address(), self._check_generate_container_die)

		try:
			self.docker_commands.deploy(container_name=container_name, infos_application=values)
		except Exception as e:
			self.container_deploy_start.remove(container_process)
			self.channel_connections[container_process].close()
	...
	def _check_generate_container_die(self, event):
		...
		def haproxy(self, container_name, args):
			if container_name is not None and args['container_process'] in self.container_deploy_start:
				dns = None

				try:
					address_container = self.docker_commands.address_container(
						container_name)
				except Exception as e:
					raise Exception(e)

				try:
					ports_container = self.docker_commands.ports_container(
						container_name)
				except Exception as e:
					raise Exception(e)
				...
				try:
					self.haproxy.generate_conf(application_name=args['application_name'],
											   container_name=container_name,
				...
				self.container_deploy_start.remove(args['container_process'])
				self.channel_connections[args['container_process']].close()

		if 'status' in ev['docker']:
			if 'start' in ev['docker']['status']:
				resp = threading.Timer(
					10, haproxy, [self, container_name, ev['args']])
				resp.daemon = True
				resp.start()

			if 'die' in ev['docker']['status']:
				if container_name is not None:
					if ev['args']['container_process'] in self.container_deploy_start:
						self.container_deploy_start.remove(
							ev['args']['container_process'])
					self.channel_connections[ev['args']
											 ['container_process']].close()

	def _load_image(self, path):
		infos = path.split('/')
		image_name = infos[len(infos) - 1:][0].split('.')[0]

		try:
			exists_image = self.docker_commands.exist_image(
				image_name=image_name)
		except Exception as e:
			raise Exception(e)

		if exists_image is False:
			try:
				self.docker_commands.load_image(path=path)
			except Exception as e:
				raise Exception(e)
	...
	def _generate_image(self, path, image_name, key_etcd, iq_response, ifrom):
		...
		try:
			self.docker_commands.deploy(
				container_name=image_name, infos_application=values, image_create=True)
		except Exception as e:
	...
	def _check_generate_image(self, event):
		...
		def basic_gi(container_name, path):
			logging.info(
				'Running the generate_image.sh with image name: %s' % (container_name))
			command = ['/usr/bin/sh', 'generate_image.sh',
					   container_name, path, 'v1', 'hello_world']
		...
		if 'status' in ev['docker']:
			if ev['docker']['status'] == 'start':
				resp = threading.Timer(
					5, check_image, [self, ev['args'], container_name])
				resp.daemon = True
				resp.start()
...
#                    self.channel_connections[container].close()
	...
	def _deploy_container(self, ifrom, application_name, container_name, key_application, iq_response):
		...
		args = {'from': str(ifrom), 'application_name': str(application_name), 'container_name': str(
				container_name), 'iq_response': str(iq_response), 'protocol': values['protocol'], 'dns': str(values['dns'])}
		...
		self.channel_connections[container_name].register(
			self.docker_process.public_address(), self._check_deploy_container)

		try:
			self.docker_commands.deploy(
				container_name=container_name, infos_application=values)
		except Exception as e:
			self.container_deploy_start.remove(container_name)
			self.channel_connections[container_name].close()
			raise Exception(e)
	...
	def _check_deploy_container(self, event):
		...
		def haproxy(container_name, args):
			...
			try:
				self.haproxy.generate_conf(application_name=args['application_name'], container_name=container_name,
										   ports_container=ports_container, protocol=args['protocol'], address_container=address_container, dns=dns)
			except Exception as e:
				raise Exception(e)
		...
		if 'status' in ev['docker']:
			if 'start' in ev['docker']['status']:
				resp = threading.Timer(10, send_response, [
					self, ev['args'], container_name])
				resp.daemon = True
				resp.start()
	...
	def muc_online(self, presence):
		if len(presence['muc']['nick'].strip()) > 0:
			if presence['muc']['nick'] != self.nick:
				...
				#if 'zeus' in presence['muc']['jid']:
				#	self.zeus = presence['muc']['jid']
