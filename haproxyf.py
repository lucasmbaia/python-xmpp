...
#
	...
	def _tcp_and_udp(self, application_name, key_ports, container_name, address_container, dns, protocol):
		keys_exists = self.etcd_conn.keys_exists(self.key + application_name)
		...
		if key_exists:
			try:
				...
				for key in keys_ports:
					for host in values['hosts']:
						if 'portSRC' in host and host['portSRC'] == key:
							for dst in ports_container[key]:
								host['containers'].append({'name': container_name, 'address': address_container + ':' + dst})
			...
			for key in keys_ports:
				...
				for dst in ports_container[key]:
					containers.append({'name': container_name, 'address': address_container + ':' + dst})
