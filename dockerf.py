...
class DockerCommands:
    ...
    def deploy(self, container_name, infos_application, image_create=False):
        ...
        try:
            command = self._deploy_command(
                container_name=container_name, values=infos_application, image_create=image_create)
        except Exception as e:
            raise Exception(e)
    ...
    def generate_image(self, image_name, args=None, ports=None, cpus="0.1", memory="10m"):
        ...
        infos_image = {'image': self.image_base,
        ...
        try:
            resp = self.deploy(container_name=image_name,
                               infos_application=infos_image, image_create=True)
        except Exception as e:
            raise Exception(e)
    ...
    def ports_container(self, container_name):
        ...
        command = ["docker", "inspect",
                   "--format='{{range $p, $conf := .NetworkSettings.Ports}}{{$p}}:{{(index $conf 0).HostPort}}-{{end}}'", container_name]
        ports_container = {}
